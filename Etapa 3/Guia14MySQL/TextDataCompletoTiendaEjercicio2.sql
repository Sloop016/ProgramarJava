...
select  p.codigo_fabricante from fabricante f , producto p where f.codigo = p.codigo_fabricante;
...
-- 8. Lista los nombres de los productos ordenados en primer lugar por el nombre de forma descendente y en segundo lugar por el precio de forma descendente.
...
select p.* from producto p, fabricante f where f.codigo = p.codigo_fabricante and f.nombre like '%lenovo%';
-- 5. Devuelve una lista de todos los productos del fabricante Crucial que tengan un precio mayor que $200.
select p.* from producto p, fabricante f where f.codigo = p.codigo_fabricante and f.nombre like '%crucial%' and p.precio >200;
-- 6. Devuelve un listado con todos los productos de los fabricantes Asus, Hewlett-Packard. Utilizando el operador IN.
select p.* from producto p, fabricante f where f.codigo = p.codigo_fabricante and f.nombre in ('Asus','Hewlett-Packard');
...
select f.*,p.* from producto p right join fabricante f on f.codigo = p.codigo_fabricante ;
-- 2. Devuelve un listado donde sólo aparezcan aquellos fabricantes que no tienen ningún
-- producto asociado.
select f.*,p.* from producto p right join fabricante f on f.codigo = p.codigo_fabricante where p.nombre is null ;
...
select * from producto where precio = (select max(precio) from producto where codigo_fabricante = (select codigo from fabricante where nombre = 'lenovo' )) and codigo_fabricante = (select codigo from fabricante where nombre = 'lenovo');
...
select f.nombre, count(*) from fabricante f, producto p where p.codigo_fabricante = f.codigo group by f.nombre having count(*) = (select count(*) from producto where codigo_fabricante = (select codigo from fabricante where nombre = 'lenovo'));