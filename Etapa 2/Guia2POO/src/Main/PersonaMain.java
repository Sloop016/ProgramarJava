...
public class PersonaMain {
    ...
    public static void main(String[] args) {
        ...
        int mayor = 0, menor = 0;
        int encima = 0, debajo = 0, ideal = 0;

        for (int i = 0; i < 4; i++) {
            pers[i] = sv.crearPersona();

            if (sv.esMayor(pers[i])) {
                mayor++;
            } else {
                menor++;
            }

            switch (sv.calcularIMC(pers[i])) {
                case -1:
                    debajo++;
                    break;
                case 0:
        ...
        System.out.println("Mayores: " + (mayor*100)/(mayor+menor) + "%");
        System.out.println("Menores: " + (menor*100)/(mayor+menor) + "%");
        System.out.println("Con peso ideal: " + (ideal*100)/(ideal+encima+debajo) + "%");
        System.out.println("Por debajo del peso ideal: " + (debajo*100)/(ideal+encima+debajo) + "%");
        System.out.println("Por encima del peso ideal: " + (encima*100)/(ideal+encima+debajo) + "%");
    }
}
